...
axios.get(url).then((res: AxiosResponse) => {
    ...
    rankingTableRows.each((i: number, elem:object) => {
        ...
        const position: number = parseInt($(elem).find(`tr:nth-child(${i+1}) > td:nth-child(2)`).text());
        // Find and store the drivers first name
        const firstName: string = $(elem).find(`tr:nth-child(${i+1}) > td:nth-child(3) .hide-for-tablet`).text().trim();
        // Find and store the drivers last name
        const lastName: string = $(elem).find(`tr:nth-child(${i+1}) > td:nth-child(3) .hide-for-mobile`).text().trim();
        ...
        const car: string = $(elem).find(`tr:nth-child(${i+1}) > td:nth-child(5)`).text().trim();
